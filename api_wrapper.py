...
def handle_process(data, config):
    ...
    # Initialize processor with full config so it can see directories,
    # summarization settings, and search model configuration.
    processor = DocumentProcessor(config)

    # Process the file (single-file pipeline)
    result = processor.process_file(Path(file_path))

    # If processing failed, surface a clear error to the caller
    if not result or not result.get("processed"):
        return {
            'success': False,
            'processed': False,
            'file': file_path,
            'error': f'Failed to process file: {Path(file_path).name}'
        }
